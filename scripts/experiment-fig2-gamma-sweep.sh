...
#
# 5 gammas × 3 envs × 5 seeds = 75 runs.
# ppo_g0_999_n256_sparse (gamma=0.999) runs are shared with fig1 — skipped automatically.
#
...
for idx in "${!all_commands[@]}"; do
  ...
  run_commands+=("mkdir -p ${run_dir} && touch ${run_dir}/LOCK && .venv/bin/python algorithm.py ${rest} --exp-name ${exp_name} --track --wandb-group fig2 >> ${run_dir}/run.log 2>&1")
  run_names+=("${run_name}")
done
